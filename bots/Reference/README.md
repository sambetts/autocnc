...
## Knowing what they are sending, and what beats it

Two gaps decided most of the champion's losses at d1a695b. In 16 fights (8 on the training map,
8 on the `hard-16-9` benchmark), **77% of the value lost went to unit types first seen less than
30 seconds before they started shooting**, and 52% to types first seen within 20 cells of home.
Their first attack landed before their base was found in 14 of the 16. Scouting stopped as soon as
the base was found, so aircraft, artillery and new tech were first seen when they arrived. The
side lost all 10 of the fights in which the enemy fielded orcas or helicopters. It had seen their
helipad 12 and 18 minutes before the aircraft came in two of them, and did nothing with it.

Three pieces now close that loop:

- **A watcher.** [`Modes/IntelWatch.cs`](Modes/IntelWatch.cs) and
  [`Logic/IntelWatchLogic.cs`](Logic/IntelWatchLogic.cs). Once their base is known, the first live
  rifleman becomes the side's watcher in every doctrine, via the `DefendOrWatchMode` and
  `AttackOrWatchMode` wrappers. It holds two fifths of the way from home to their base
  (`intel.picket`), falls back when threatened (`intel.evade`), and every 100 s runs to 7 cells
  short of their base to see what it is building and fielding (`intel.rescout`, and
  `intel.rescout-detour` around a threat). Every other rifleman keeps its doctrine's mode. It
  is a rifleman on purpose. A jeep or buggy watcher took the only fast escort a Nod opening has
  off its harvesters, and every such version lost the Nod mirror on `hard-16-9` that the
  champion wins.
- **A threat memory.** [`Modes/EnemySightings.cs`](Modes/EnemySightings.cs) now keeps every enemy
  combat unit seen, by actor id, at its last-seen value and tick, and every enemy structure type
  seen. `RecentThreats` sums the last two minutes by type. It also counts whatever just hit us,
  seen or not, through `RecordAttacker` from the damage callback the guide sanctions: that is how
  artillery firing from beyond sight enters the mix at all. A helipad seen before any aircraft adds
  anticipated orcas and helicopters.
- **Measured counters.** [`Logic/MatchupTable.cs`](Logic/MatchupTable.cs) is generated by
  `scripts/export-bot-matchups.ps1` from the duel lab: every combat unit against every other,
  equal cost, fought by the engine over three seeds. See
  [`docs/unit-matchups.md`](../../docs/unit-matchups.md) and `tools/DuelLab`.
  [`Logic/CounterLogic.cs`](Logic/CounterLogic.cs) scores each unit the side can build now by its
  margin against the threat mix, weighted by value. `TrainUnitsMode` folds the answer into the
  plan directly after the last harvester rung, so income and the siege core keep priority:
  - **Aircraft** get their own rule (`production.counter-air`). Once an orca or helicopter is
    seen, or a helipad, the best answer to the aircraft alone is bought until the side's
    anti-air is worth 80% of them. That is e3 or apc for GDI, and e3, mlrs or bike for Nod.
  - **Everything else** is countered (`production.counter-rung`, `production.counter-endless`)
    only when the army standing is losing to what it faces (value-weighted score of -20 per 100
    or worse), and only by an answer at least 30 points better than what is standing.

The lab's margins come from massed, equal-cost fights on open ground, and that limits how far the
table can be trusted in a real match. The first versions of this section let the table rewrite
production freely. They bought buggies one at a time against rocket infantry the lab says they
beat at +62 per 100, and each one died within seconds. They also swapped the Nod siege rung's arty
for flame tanks, and lost the matches the champion wins with 26 arty behind 100 riflemen. So the
siege rungs are protected, and small margins are treated as noise.

Using the table is fair play: it is static rules knowledge, the same a player reads from the
encyclopedia, measured instead of assumed. The two-bin `ArmyMixLogic` rule still runs first.

**Measured result: not promoted.** Nine variants were played on `hard-16-9` against the champion
d1a695b. Its results there are deterministic: 2 of 8 won, mean fitness 0.632.

| variant | wins | mean fitness | what changed |
|---|---|---|---|
| v1 | 1 | 0.500 | buggy watcher, free counters inserted after the first harvester rung |
| v2 | 0 | 0.489 | counters after the last harvester rung, siege rungs protected |
| v3 | 1 (+1 timed out) | 0.594 | v2 without the watcher |
| v4 | 1 | 0.535 | v3 with the watcher back and a replacement buggy, counters need a 30-point gap |
| v5 | 1 | 0.522 | counters only while the army is losing, watcher with a replacement buggy |
| v6 | 2 | 0.632 | v5 without the watcher: identical to the champion, counters never fired |
| **v7** | **2** | **0.611** | **this code**: rifleman watcher, anti-air rule, gated counters |
| v8 | 1 | 0.522 | v7 with the picket at 30% and no base-memory writes |
| v9 | 1 | 0.591 | v7 without the watcher |

On the 16-match `hard-16-9-wide`, v7 played head to head with the champion won 3 against 5. The
median paired fitness delta was -0.017, and it was ahead in 7 pairs to 9. That is a restore.

The effect splits by faction. As GDI it was level with the champion (2 wins each, mean delta
+0.024). It won both GDI v Nod seeds 300001 and 300002, which the champion loses: rockets and
APCs bought against the helicopters and orcas those losses died to. As Nod it was clearly worse
(1 win against 3, mean delta -0.141). Every variant with a watcher lost the Nod mirror at seed
300008, which the champion wins with 26 arty behind 117 riflemen. `IntelWatch.Enabled` is the
switch for measuring that.
