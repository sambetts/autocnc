...
failing `reason:` check means a code path never executed at all.
  ...
  "checks": [
    {
      ...
      "query": "reason:escorting harvester under threat",
      "operator": ">=",
      "value": "1"
    },
    {
...
`units.mean(<field>,type=x)`, and `reason:<literal>` for the count of decisions whose reason
contains that literal. Operators: `>=`, `>`, `<=`, `<`, `==`, `!=`, `contains`, `present`, `absent`.

If you added or changed a code path, give it a reason literal nothing else uses and assert it with
a `reason:` check. That is the only thing that distinguishes "the new branch is wrong" from "the
new branch never ran", and confusing those two is how the same class of bug gets re-introduced
under a new name.
