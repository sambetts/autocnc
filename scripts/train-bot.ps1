...
.DESCRIPTION
    ...
    {run}, in both arguments and stdin. The rendered prompt is larger than Windows allows on a
    command line, so the built-in Copilot configuration sends it in on standard input. The built-in
    configuration grants access to {evidence}, but not to the launcher's reversible source snapshot
    stored elsewhere in the run.
