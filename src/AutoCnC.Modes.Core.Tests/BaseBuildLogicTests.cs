...
namespace AutoCnC.Modes.Core.Tests
{
	...
	public class BaseBuildLogicTests
	{
		...
		{
			Assert.That(BaseBuildLogic.ChooseNext(State(), BaseBuildLogic.DefaultPlan), Is.EqualTo("powr"));
		}
		...
		public void MovesToTheNextStepOnceSatisfied()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(State(owned: owned), BaseBuildLogic.DefaultPlan),
		...
		public void PrioritisesPowerDuringABrownout()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(State(power: -20, owned: owned), BaseBuildLogic.DefaultPlan),
		...
		public void PicksWhicheverFactionVariantIsActuallyBuildable()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(nod, BaseBuildLogic.DefaultPlan), Is.EqualTo("proc"));

			var withRefinery = State(buildable: ["nuke", "proc", "hand"],
				owned: new Dictionary<string, int> { ["nuke"] = 2, ["proc"] = 1 });

			Assert.That(BaseBuildLogic.ChooseNext(withRefinery, BaseBuildLogic.DefaultPlan), Is.EqualTo("hand"),
		...
		public void SkipsStepsItCannotBuildYet()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(state, BaseBuildLogic.DefaultPlan), Is.EqualTo("proc"),
		...
		public void ReturnsNullWhenThePlanIsComplete()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(State(owned: owned), BaseBuildLogic.DefaultPlan), Is.Null);
		}

		[Test]
		public void ReturnsNullWhenNothingIsBuildable()
		{
			Assert.That(BaseBuildLogic.ChooseNext(State(buildable: []), BaseBuildLogic.DefaultPlan), Is.Null);
		...
		public void CountsAreCaseInsensitive()
		{
			...
			Assert.That(BaseBuildLogic.ChooseNext(State(owned: owned), BaseBuildLogic.DefaultPlan), Is.EqualTo("proc"));
		}
