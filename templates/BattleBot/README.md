...
After a fight, **Analyze & improve** can ask your configured local coding agent to make a
reversible improvement from the saved battle evidence. **History & trends** compares KPIs across
iterations; **Continuous improvement** evaluates each candidate against a paired control and
promotes it or restores the saved champion before continuing.

Start with `Doctrines/StarterDoctrine.cs` for the build and production plan,
`Logic/StarterLogic.cs` for pure decisions, and `Modes/StarterMode.cs` for the engine-facing
sense/act layer.
